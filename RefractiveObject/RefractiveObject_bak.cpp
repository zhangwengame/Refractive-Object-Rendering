                                      ...
                                      void* pUserContext )
{
	...
	V_RETURN(g_Mesh11.Create(pd3dDevice, L"tiny\\k.sdkmesh", true));
	//========== ===========
	//为了最终成像
	...
	V_RETURN(DXUTFindDXSDKMediaFileCch(strPath, MAX_PATH, L"Light Probes\\stpeters_cross.dds"));
...
//	XMFLOAT4 Color;
//};
//ID3D11RenderTargetView*     resTV;
//ID3D11Texture2D*			resTex = NULL;
//ID3D11Texture2D*			resTexCopy = NULL;
//
//void renderToBackground(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext){
...
void renderToBackTexture(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext){
	...
	//hr = pd3dDevice->CreateShaderResourceView(tmp, &backTexRVD, &imgTextureRVBack);
	//if (FAILED(hr))
	//	return;

	//D3DX11SaveTextureToFile(pd3dImmediateContext, resTexBackCopy, D3DX11_IFF_BMP, L"newback.bmp");
...
void renderLight(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext){
	...
	//}
	//else if (type == CAMPOSITION){
	//	pd3dImmediateContext->IASetVertexBuffers(0, 1, &seeVertexBuf, &stride, &offset);
	//	pd3dImmediateContext->IASetIndexBuffer(seeIndexBuf, DXGI_FORMAT_R32_UINT, 0);
	//	dUp.x = 0.0f;
	//	dUp.y = 0.0f;
	//	dUp.z = 1.0f;
	//}
	...
	{
		FILE *stream;
		if ((stream = fopen("pos.data", "wb")) == NULL) /* open file TEST.$$$ */
		{
			fprintf(stderr, "Cannot open output file.\n");
			return;
		}
		fwrite(posLight, sizeof(posLight), 1, stream); /* 写的struct文件*/
		fclose(stream); /*关闭文件*/
	}
	...
	//fAmbient = 0.1f;
	//pPerFrame->m_vLightDirAmbient = D3DXVECTOR4(vecEye.x, vecEye.y, vecEye.z, fAmbient);
	//pd3dImmediateContext->Unmap(g_pcbPSPerFrame, 0);
	//pd3dImmediateContext->PSSetConstantBuffers(g_iCBPSPerFrameBind, 1, &g_pcbPSPerFrame);

	//dUp = (0.5f, 0.5f, 1.0f);
	//pd3dImmediateContext->IASetVertexBuffers(0, 1, &vertexBuf, &stride, &offset);
	//pd3dImmediateContext->IASetIndexBuffer(indexBuf, DXGI_FORMAT_R32_UINT, 0);
	...
	//D3DXMATRIX dView;
	//D3DXVECTOR3 dEye(vecEye.x, vecEye.y, vecEye.z);
	//D3DXVECTOR3 dAt(0.0f, 0.0f, 0.0f);
	////D3DXVECTOR3 dUp(0.5f, 0.5f, 1.0f);
	//D3DXMatrixLookAtLH(&dView, &dEye, &dAt, &dUp);
	//D3DXMATRIX dWorld;
	...
	{
		FILE *stream;
		if ((stream = fopen("dir.data", "wb")) == NULL) /* open file TEST.$$$ */
		{
			fprintf(stderr, "Cannot open output file.\n");
			return;
		}
		fwrite(dirLight, sizeof(dirLight), 1, stream); /* 写的struct文件*/
		fclose(stream); /*关闭文件*/
	}
	pd3dImmediateContext->OMSetRenderTargets(1, &pRTV, pDSV);
...
void renderView(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext){
	...
	{
		FILE *stream;
		if ((stream = fopen("seepos.data", "wb")) == NULL) /* open file TEST.$$$ */
		{
			fprintf(stderr, "Cannot open output file.\n");
			return;
		}
		fwrite(posView, sizeof(posView), 1, stream); /* 写的struct文件*/
		fclose(stream); /*关闭文件*/
	}
	...
	{
		FILE *stream;
		if ((stream = fopen("seedir.data", "wb")) == NULL) /* open file TEST.$$$ */
		{
			fprintf(stderr, "Cannot open output file.\n");
			return;
		}
		fwrite(dirView, sizeof(dirView), 1, stream); /* 写的struct文件*/
		fclose(stream); /*关闭文件*/
	}
	pd3dImmediateContext->OMSetRenderTargets(1, &pRTV, pDSV);
                                  ...
                                  float fElapsedTime, void* pUserContext )
{
	...
	//cudaError_t cudaStatus = cudaMalloc((void**)&prephotonrad_d, 512 * 512 * 3 * sizeof(float));
	//cudaStatus = cudaMemcpy(prephotonrad_d, seephotonrad, 512 * 512 * 3 * sizeof(float), cudaMemcpyHostToDevice);
	if (seephotonrad) free(seephotonrad);
	gaussian2D(prephotonrad_d, &seephotonrad, 512, 13, 13.0 / 6, 1);
	FILE *stream;
	if ((stream = fopen("rad.data", "wb")) == NULL) /* open file TEST.$$$ */
	{
		fprintf(stderr, "Cannot open output file.\n");
		return;
	}
	fwrite(seephotonrad, 512 * 512 * 3 * sizeof(float), 1, stream); /* Ð´µÄstructÎÄ¼þ*/
	fclose(stream);
	//-------------------------
